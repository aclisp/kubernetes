...
func (r *randomAllocator) GetState() string {
	return ""
}

func (r *randomAllocator) SetState(_ string) {
	// noop
}
...
func (r *rangeAllocator) GetState() string {
	r.lock.Lock()
	defer r.lock.Unlock()
	return r.used.Text(16)
}

func (r *rangeAllocator) SetState(s string) {
	r.lock.Lock()
	defer r.lock.Unlock()
	_, ok := r.used.SetString(s, 16)
	if !ok {
		log.Fatalf("Unable to restore PortRangeAllocator's state from (%s)", s)
	}
}
